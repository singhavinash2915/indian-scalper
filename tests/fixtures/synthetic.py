"""Synthetic OHLCV generators for indicator + scoring tests.

Deterministic (seeded). Each scenario is engineered so the scoring engine
produces a known outcome:

* ``bullish_breakout_df`` — long flat squeeze then a moderate uptrend
  with a final-bar volume spike. Tuned so RSI lands in the entry range
  (55–75) rather than above the hard-block threshold (78). Reliably
  scores ≥ 4/8 without triggering the hard block.
...
def bullish_breakout_df(n_bars: int = 120, seed: int = 7) -> pd.DataFrame:
    ...
    # Linear ramp of ~0.4 per bar + noise with std 1.0 → bar returns have
    # mean ≈ 0.4, std ≈ 1.4 → P(up bar) ≈ 0.61 → RSI(14) stabilises around
    # 60–72 (comfortably inside the entry band, below the 78 hard block).
    ...
    # Final-bar volume spike: 4× the trend-phase average → forces the
    # volume_surge factor to fire on the scored bar.
